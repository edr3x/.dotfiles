...
local volume_osd_icon = wibox.widget({
    {
        id = "popup_icon",
        markup = helpers.colorize_text(" ", beautiful.foreground),
...
screen.connect_signal("request::desktop_decoration", function(s)
    ...
    awesome.connect_signal("signal::volume", function(value, muted)
        ...
        if muted == 1 or value == 0 then
            volume_osd_icon.popup_icon:set_markup_silently(helpers.colorize_text("", beautiful.blacks))
            volume_osd_bar.volume_osd_progressbar.color = beautiful.blacks
        else
            volume_osd_icon.popup_icon:set_markup_silently(helpers.colorize_text("", beautiful.white))
            volume_osd_bar.volume_osd_progressbar.color = beautiful.cyan
        end
